...
export const CreateStoreCategory = (props) => {
  let filter = getQueryStringValue("filter");
  if (filter) {
    filter = JSON.parse(filter);
  }
  ...
  return (
    <Create {...props} title="ra.strings.create_store_category">
      <SimpleForm
        redirect="show"

        // validate={validate}
      >
        <TextInput source="name" label="ra.strings.name_en" />
        <TextInput source="name_ar" label="ra.strings.name_ar" />
