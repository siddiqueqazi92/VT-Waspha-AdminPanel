...
const SubcategoryInput = (props) => {
  ...
  if (values.category && values.category.id && props.choices) {
    const selectedCategory = props.choices.filter(
      (subcategory) => subcategory.parent_id === values.category.id
    );
...
export const EditAd = (props) => {
  ...
  return (
    <Edit {...props}>
      ...
      <SimpleForm redirect="list" toolbar={<EditToolbarNoDeleteButton />}>
